...
#define COMPUTE_VOLUME_FLUX_PDS(flux, icell, ipoint)                          \
  ...
  flux[ind++] = U_ * q - AVcoeff * qx;                                        \
  flux[ind] = V_ * q - AVcoeff * qy
...
#define COMPUTE_NUMERICAL_FLUX_PDS(flux, owner_cell, neighbor_cell, ipoint) \
  ...
  flux[ind++] = 0.5 * (U_ * (q_o + q_n) - nx * diff -                       \
                       (AVcoeff_o * qx_o + AVcoeff_n * qx_n));              \
  flux[ind] = 0.5 * (V_ * (q_o + q_n) - ny * diff -                         \
                     (AVcoeff_o * qy_o + AVcoeff_n * qy_n))

namespace deneb {
// ------------------------------- Constants ------------------------------- //
ConstantsAdvection2D::ConstantsAdvection2D() {
  auto& config = AVOCADO_CONFIG;
  U_ = std::stod(config->GetConfigValue("AdvectionSpeed.0"));
  V_ = std::stod(config->GetConfigValue("AdvectionSpeed.1"));
  Speed_ = std::sqrt(U_ * U_ + V_ * V_);
}
// ------------------------------- Equation -------------------------------- //
EquationAdvection2D::EquationAdvection2D()
    : ConstantsAdvection2D(), Equation(D_, S_, false) {
  MASTER_MESSAGE(avocado::GetTitle("EquationAdvection2D"));
  ...
  auto& config = AVOCADO_CONFIG;
  problem_ = ProblemAdvection2D::GetProblem(config->GetConfigValue(PROBLEM));
  const std::string& numflux = config->GetConfigValue(CONVECTIVE_FLUX);
  if (!numflux.compare("Upwind")) {
    ASSIGN_FLUX(EquationAdvection2D, Upwind);
  } else
...
void EquationAdvection2D::RegistBoundary(const std::vector<int>& bdry_tag) {
  auto& config = AVOCADO_CONFIG;
  ...
  for (auto&& tag : all_bdry_tag) {
    ...
    if (boundary_registry_.find(tag) == boundary_registry_.end())
      boundary_registry_[tag] =
          BoundaryAdvection2D::GetBoundary(bdry_type, tag, this);
  }
}
void EquationAdvection2D::BuildData(void) {
  MASTER_MESSAGE(avocado::GetTitle("EquationAdvection2D::BuildData()"));
...
void EquationAdvection2D::GetCellPostSolution(
...
void EquationAdvection2D::GetFacePostSolution(
...
void EquationAdvection2D::ComputeInitialSolution(double* solution,
                                                 const double t) {
  const int& order = DENEB_DATA->GetOrder();
  const int& num_bases = DENEB_DATA->GetNumBases();
  const int& num_cells = DENEB_DATA->GetNumCells();
...
void EquationAdvection2D::ComputeLocalTimestep(
  ...
  for (int icell = 0; icell < num_cells; icell++) {
    ...
    local_timestep[icell] =
        cell_volumes[icell] /
        (U_ * Vx + V_ * Vy + AVcoeff * dt_auxiliary_[icell]);
  }
  avocado::VecScale(num_cells, 1.0 / static_cast<double>(2 * order + 1),
                    &local_timestep[0]);
}
void EquationAdvection2D::ComputeRHS(const double* solution, double* rhs,
                                     const double t) {
  static const int& num_bases = DENEB_DATA->GetNumBases();
  static const int& num_cells = DENEB_DATA->GetNumCells();
  static const int sb = S_ * num_bases;
...
void EquationAdvection2D::ComputeSystemMatrix(const double* solution,
...
void EquationAdvection2D::ComputeComFlux(
...
void EquationAdvection2D::ComputeComFluxJacobi(
...
void EquationAdvection2D::ComputeNumFluxUpwind(const int num_points,
  ...
  for (int ipoint = 0; ipoint < num_points; ipoint++) {
    ...
    const double S = std::abs(U_ * nx + V_ * ny);

    const auto diff = S * (q_n - q_o);

    COMPUTE_NUMERICAL_FLUX_PDS(flux, owner_cell, neighbor_cell, ipoint);
  }
}
void EquationAdvection2D::ComputeNumFluxJacobiUpwind(const int num_points,
                                                     FACE_JACOBI_OUTPUTS,
                                                     FACE_INPUTS) {
  // flux_jacobi(ds1s2) = F(ds1) over U(s2)
  // flux_grad_jacobi(d1s1 s2d2) = F(d1s1) over gradU(d2s2)
...
// ------------------------------- Boundary -------------------------------- //
std::shared_ptr<BoundaryAdvection2D> BoundaryAdvection2D::GetBoundary(
    const std::string& type, const int bdry_tag,
    EquationAdvection2D* equation) {
  return nullptr;
}
// -------------------------------- Problem -------------------------------- //
std::shared_ptr<ProblemAdvection2D> ProblemAdvection2D::GetProblem(
    const std::string& name) {
  if (!name.compare("DoubleSine"))
    return std::make_shared<DoubleSineAdvection2D>();
  else if (!name.compare("SquareWave"))
    return std::make_shared<SquareWaveAdvection2D>();
  else
...
// Problem = DoubleSine
// ProblemInput = -
DoubleSineAdvection2D::DoubleSineAdvection2D() : wave_number_({1.0, 1.0}) {
...
void DoubleSineAdvection2D::Problem(const int num_points,
  ...
  for (int ipoint = 0; ipoint < num_points; ipoint++) {
    ...
    solutions[ind++] =
        1.0 + 0.2 * std::sin(2.0 * M_PI * wave_number_[0] * (x - U_ * time)) *
                  std::sin(2.0 * M_PI * wave_number_[1] * (y - V_ * time));
  }
}
// Problem = SquareWave
// ProblemInput = -
SquareWaveAdvection2D::SquareWaveAdvection2D()
    : x_edge_({0.25, 0.75}), y_edge_({0.25, 0.75}) {
  MASTER_MESSAGE("SquareWave problem\n");

  std::stringstream str;
  str << "\tInput (fixed):\n";
  str << "\t\tx-edge = " << x_edge_[0] << ", " << x_edge_[1] << "\n";
  str << "\t\ty-edge = " << y_edge_[0] << ", " << y_edge_[1] << "\n";
}
void SquareWaveAdvection2D::Problem(const int num_points,
                                    std::vector<double>& solutions,
                                    const std::vector<double>& coord,
                                    const double time) const {
  int ind = 0;
  for (int ipoint = 0; ipoint < num_points; ipoint++) {
    ...
    if (x > x_edge_[0] && x < x_edge_[1] && y > y_edge_[0] && y < y_edge_[1])
      solutions[ind++] = 1.0;
    else
      solutions[ind++] = 0.1;
  }
