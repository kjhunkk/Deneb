...
#define COMPUTE_NUMERICAL_FLUX_PDS(flux, ipoint)                              \
  ...
  flux[ind++] = 0.5 * (du_o * w_o + du_n * w_n - nx * diff4);                 \
  flux[ind++] = 0.5 * ((dE_o + p_o) * u_o + (dE_n + p_n) * u_n - nx * diff5); \
  flux[ind++] = 0.5 * (dv_o + dv_n - ny * diff1);                             \
  flux[ind++] = 0.5 * (dv_o * u_o + dv_n * u_n - ny * diff2);                 \
  flux[ind++] = 0.5 * (dv_o * v_o + p_o + dv_n * v_n + p_n - ny * diff3);     \
  flux[ind++] = 0.5 * (dv_o * w_o + dv_n * w_n - ny * diff4);                 \
  flux[ind++] = 0.5 * ((dE_o + p_o) * v_o + (dE_n + p_n) * v_n - ny * diff5); \
  flux[ind++] = 0.5 * (dw_o + dw_n - nz * diff1);                             \
  flux[ind++] = 0.5 * (dw_o * u_o + dw_n * u_n - nz * diff2);                 \
  flux[ind++] = 0.5 * (dw_o * v_o + dw_n * v_n - nz * diff3);                 \
  flux[ind++] = 0.5 * (dw_o * w_o + p_o + dw_n * w_n + p_n - nz * diff4);     \
  flux[ind] = 0.5 * ((dE_o + p_o) * w_o + (dE_n + p_n) * w_n - nz * diff5)

namespace deneb {
  // ------------------------------- Constants -------------------------------
  // //
  ConstantsEuler3D::ConstantsEuler3D()
      : r_(std::stod(AVOCADO_CONFIG->GetConfigValue(HEAT_CAPACITY_RATIO))),
        r_inv_(1.0 / r_),
        rm1_(r_ - 1.0),
        rm1_inv_(1.0 / rm1_) {}
  double ConstantsEuler3D::ComputePressure(const double* sol) const {
    return rm1_ * (sol[S_ - 1] -
                   0.5 / sol[0] * avocado::VecInnerProd(D_, &sol[1], &sol[1]));
  }
  double ConstantsEuler3D::ComputeTotalEnergy(const double* pri) const {
  ...
  // ------------------------------- Equation --------------------------------
  // //
  EquationEuler3D::EquationEuler3D()
      : ConstantsEuler3D(), Equation(D_, S_, false) {
    MASTER_MESSAGE(avocado::GetTitle("EquationEuler3D"));
    ...
    problem_ = ProblemEuler3D::GetProblem(config->GetConfigValue(PROBLEM));
    const std::string& numflux = config->GetConfigValue(CONVECTIVE_FLUX);
    if (!numflux.compare("LLF")) {
      ASSIGN_FLUX(EquationEuler3D, LLF);
    } else if (!numflux.compare("Roe")) {
      ASSIGN_FLUX(EquationEuler3D, Roe);
    } else
  ...
  void EquationEuler3D::RegistBoundary(const std::vector<int>& bdry_tag) {
    auto& config = AVOCADO_CONFIG;
    ...
    for (auto&& tag : all_bdry_tag) {
      ...
      if (boundary_registry_.find(tag) == boundary_registry_.end())
        boundary_registry_[tag] =
            BoundaryEuler3D::GetBoundary(bdry_type, tag, this);
    }
  }
  void EquationEuler3D::BuildData(void) {
    MASTER_MESSAGE(avocado::GetTitle("EquationEuler3D::BuildData()"));
  ...
  void EquationEuler3D::GetCellPostSolution(
      const int icell, const int num_points,
  ...
  void EquationEuler3D::GetFacePostSolution(
      const int num_points, const std::vector<double>& solution,
      const std::vector<double>& solution_grad,
      const std::vector<double>& normal, std::vector<double>& post_solution) {
    int ind = 0;
    for (int ipoint = 0; ipoint < num_points; ipoint++) {
      for (int istate = 0; istate < S_; istate++)
        post_solution[ind++] = solution[ipoint * S_ + istate];
  ...
  void EquationEuler3D::ComputeInitialSolution(double* solution,
                                               const double t) {
    ...
    for (int icell = 0; icell < num_cells; icell++) {
      DENEB_DATA->GetCellQuadrature(icell, 2 * order, quad_points,
                                    quad_weights);
        ...
        for (int ibasis = 0; ibasis < num_bases; ibasis++)
          for (int ipoint = 0; ipoint < num_points; ipoint++)
            solution[icell * sb + istate * num_bases + ibasis] +=
                ...
                basis_values[ipoint * num_bases + ibasis] *
                quad_weights[ipoint];
    }
  }
  void EquationEuler3D::ComputeLocalTimestep(
    ...
    for (int icell = 0; icell < num_cells; icell++) {
      ...
      const double dE =
          solution[icell * sb + 4 * num_bases] * cell_basis_value[icell][0];
  ...
  bool EquationEuler3D::IsContact(
      const int& icell, const std::vector<int>& neighbor_cells,
      const double* solution, const double* total_solution) const {
    static const int& num_bases = DENEB_DATA->GetNumBases();
    static const int sb = S_ * num_bases;
    ...
    const double owner_p =
        rm1_ * (sol[4] - 0.5 * (sol[1] * sol[1] + sol[2] * sol[2] + sol[3] * sol[3]) / sol[0]);
    ...
    for (auto&& cell : neighbor_cells) {
      ...
      const double neighbor_p =
          rm1_ * (sol[4] -
                  0.5 * (sol[1] * sol[1] + sol[2] * sol[2] + sol[3] * sol[3]) /
                      sol[0]);
  ...
  double EquationEuler3D::ComputeMaxCharacteristicSpeed(
  ...
  const std::vector<double>& EquationEuler3D::ComputePressureFixValues(
  ...
  void EquationEuler3D::ComputeRHS(const double* solution, double* rhs,
                                   const double t) {
    static const int& num_bases = DENEB_DATA->GetNumBases();
    static const int& num_cells = DENEB_DATA->GetNumCells();
    static const int sb = S_ * num_bases;
    ...
    for (int iface = num_inner_faces; iface < num_faces; iface++) {
      ...
      avocado::Kernel0::f4(&outer_solution_[neighbor_cell * sb],
                           ...
                           &neighbor_solution[0], S_, num_bases, num_points,
                           1.0, 0.0);
  ...
  void EquationEuler3D::ComputeSystemMatrix(const double* solution, Mat& sysmat,
                                            const double t) {
    static const int& num_bases = DENEB_DATA->GetNumBases();
    static const int& num_cells = DENEB_DATA->GetNumCells();
    static const int& num_bdries = DENEB_DATA->GetNumBdries();
    ...
    static std::vector<double> neighbor_solution(S_ * max_num_face_points_);
    static std::vector<double> neighbor_solution_grad;
    static std::vector<double> flux_neighbor_jacobi(DSS_ *
                                                    max_num_face_points_);
    ...
    for (int iface = 0; iface < num_inner_faces; iface++) {
      ...
      avocado::Kernel1::f59(&flux_derivative[0],
                            &face_owner_coefficients[iface][0], &block[0], S_,
      ...
      avocado::Kernel1::f59(&flux_derivative[0],
                            &face_neighbor_coefficients[iface][0], &block[0],
                            S_, sb, num_points * D_, num_bases, -1.0, 0.0);
    ...
    for (int ibdry = 0; ibdry < num_bdries; ibdry++) {
      ...
      for (int ipoint = 0; ipoint < num_points; ipoint++)
        cblas_dger(CBLAS_LAYOUT::CblasRowMajor, DSS_, num_bases, 1.0,
                   ...
                   &flux_derivative[ipoint * dssb], num_bases);
      avocado::Kernel1::f59(&flux_derivative[0],
                            &bdry_owner_coefficients[ibdry][0], &block[0], S_,
                            sb, num_points * D_, num_bases, 1.0, 0.0);
      MatSetValuesBlocked(sysmat, 1, &mat_index[owner_cell], 1,
                          &mat_index[owner_cell], &block[0], ADD_VALUES);
    }
    ...
    for (int iface = num_inner_faces; iface < num_faces; iface++) {
      ...
      ComputeNumFluxJacobi(
          ...
          neighbor_cell + num_cells, owner_solution, owner_solution_grad,
          neighbor_solution, neighbor_solution_grad, face_normals[iface]);
    ...
    for (int icell = 0; icell < num_cells; icell++) {
      ...
      ComputeComFluxJacobi(num_points, flux_owner_jacobi,
                           flux_owner_grad_jacobi, icell, owner_solution,
                           owner_solution_grad);
  ...
  void EquationEuler3D::ComputeComFlux(
  ...
  void EquationEuler3D::ComputeComFluxJacobi(
  ...
  void EquationEuler3D::ComputeNumFluxLLF(
  ...
  void EquationEuler3D::ComputeNumFluxJacobiLLF(
  ...
  void EquationEuler3D::ComputeNumFluxRoe(
      const int num_points, std::vector<double>& flux, FACE_INPUTS) {
    int ind = 0;
    for (int ipoint = 0; ipoint < num_points; ipoint++) {
      GET_NORMAL_PD(normal);
  ...
  void EquationEuler3D::ComputeNumFluxJacobiRoe(
      const int num_points, FACE_JACOBI_OUTPUTS, FACE_INPUTS) {
    // flux_jacobi(ds1s2) = F(ds1) over U(s2)
    int ind = 0;
    static std::vector<Dual<S_>> flux1(DS_);
  ...
  // ------------------------------- Boundary --------------------------------
  // //
  std::shared_ptr<BoundaryEuler3D> BoundaryEuler3D::GetBoundary(
      const std::string& type, const int bdry_tag, EquationEuler3D* equation) {
    if (!type.compare("Wall"))
      return std::make_shared<WallEuler3D>(bdry_tag, equation);
    else if (!type.compare("Riemann"))
      return std::make_shared<RiemannEuler3D>(bdry_tag, equation);
    else if (!type.compare("Constant"))
      return std::make_shared<ConstantBdryEuler3D>(bdry_tag, equation);
    else if (!type.compare("SupersonicInflow"))
      return std::make_shared<SupersonicInflowBdryEuler3D>(bdry_tag, equation);
    else if (!type.compare("BackPressure"))
      return std::make_shared<BackPressureBdryEuler3D>(bdry_tag, equation);
    ERROR_MESSAGE("Wrong boundary condition (no-exist):" + type + "\n");
    return nullptr;
  }
  // Boundary = Wall
  // Dependency: -
  WallEuler3D::WallEuler3D(const int bdry_tag, EquationEuler3D* equation)
      : BoundaryEuler3D(bdry_tag, equation) {
    MASTER_MESSAGE("Wall (tag=" + std::to_string(bdry_tag) + ")\n");
  }
  void WallEuler3D::ComputeBdrySolution(
  ...
  void WallEuler3D::ComputeBdryFlux(
      const int num_points, std::vector<double>& flux, FACE_INPUTS,
      const std::vector<double>& coords, const double& time) {
    int ind = 0;
    for (int ipoint = 0; ipoint < num_points; ipoint++) {
  ...
  void WallEuler3D::ComputeBdrySolutionJacobi(
  ...
  void WallEuler3D::ComputeBdryFluxJacobi(
      const int num_points, std::vector<double>& flux_jacobi,
  ...
  RiemannEuler3D::RiemannEuler3D(const int bdry_tag, EquationEuler3D* equation)
      : BoundaryEuler3D(bdry_tag, equation) {
    auto& config = AVOCADO_CONFIG;
    ...
    uf_ =
        Ma_ * std::cos(AoA_ * pi_ / 180.0) * std::cos(sideslip_ * pi_ / 180.0);
    vf_ =
        Ma_ * std::sin(AoA_ * pi_ / 180.0) * std::cos(sideslip_ * pi_ / 180.0);
  ...
  void RiemannEuler3D::ComputeBdrySolution(
      const int num_points, std::vector<double>& bdry_u,
      std::vector<double>& bdry_div_u, const std::vector<double>& owner_u,
      const std::vector<double>& owner_div_u, const std::vector<double>& normal,
      const std::vector<double>& coords, const double& time) {
    ...
    for (int ipoint = 0; ipoint < num_points; ipoint++) {
      ...
      if (std::abs(Vf) >= 1.0) {
        ...
        if (V >= 0.0) {
          // outflow
          d_temp = std::pow(
              (R_p - R_m) * (R_p - R_m) * std::pow(d, r_) / p * c3, rm1_inv_);
  ...
  void RiemannEuler3D::ComputeBdryFluxJacobi(
      const int num_points, std::vector<double>& flux_jacobi,
      std::vector<double>& flux_grad_jacobi, FACE_INPUTS,
      const std::vector<double>& coords, const double& time) {
    ...
    for (int ipoint = 0; ipoint < num_points; ipoint++)
      gemmAB(1.0, &flux_neighbor_jacobi[ipoint * DSS_],
             &bdry_u_jacobi[ipoint * SS_], 1.0, &flux_jacobi[ipoint * DSS_],
             DS_, S_, S_);
  }
  // Boundary = Constant
  // Dependency: BdryInput(num)
  // BdryInput(num) = rho, rhoU, rhoV, rhoW, rhoE
  ConstantBdryEuler3D::ConstantBdryEuler3D(const int bdry_tag,
                                           EquationEuler3D* equation)
      : BoundaryEuler3D(bdry_tag, equation) {
    ...
    std::vector<std::string> names = {"rho", "rhoU", "rhoV", "rhoW", "rhoE"};
  ...
  void ConstantBdryEuler3D::ComputeBdrySolution(
      const int num_points, std::vector<double>& bdry_u,
      std::vector<double>& bdry_div_u, const std::vector<double>& owner_u,
      const std::vector<double>& owner_div_u, const std::vector<double>& normal,
      const std::vector<double>& coords, const double& time) {
    ...
    for (int ipoint = 0; ipoint < num_points; ipoint++)
      for (int istate = 0; istate < S_; istate++)
        bdry_u[ind++] = values_[istate];
  }
  void ConstantBdryEuler3D::ComputeBdryFlux(
      const int num_points, std::vector<double>& flux, FACE_INPUTS,
      const std::vector<double>& coords, const double& time) {
  ...
  void ConstantBdryEuler3D::ComputeBdrySolutionJacobi(
      const int num_points, double* bdry_u_jacobi,
      const std::vector<double>& owner_u,
      const std::vector<double>& owner_div_u, const std::vector<double>& normal,
      const std::vector<double>& coords, const double& time) {
  ...
  void ConstantBdryEuler3D::ComputeBdryFluxJacobi(
      const int num_points, std::vector<double>& flux_jacobi,
      std::vector<double>& flux_grad_jacobi, FACE_INPUTS,
      const std::vector<double>& coords, const double& time) {
    static const int max_num_bdry_points = equation_->GetMaxNumBdryPoints();
  ...
  SupersonicInflowBdryEuler3D::SupersonicInflowBdryEuler3D(
      const int bdry_tag, EquationEuler3D* equation)
      : BoundaryEuler3D(bdry_tag, equation) {
    MASTER_MESSAGE(
        "SupersonicInflow boundary (tag=" + std::to_string(bdry_tag) + ")\n");
    ...
    sideslip_ = std::stod(config->GetConfigValue(BDRY_INPUT_I(bdry_tag, 4)));
  ...
  void SupersonicInflowBdryEuler3D::ComputeBdrySolution(
      const int num_points, std::vector<double>& bdry_u,
      std::vector<double>& bdry_div_u, const std::vector<double>& owner_u,
      const std::vector<double>& owner_div_u, const std::vector<double>& normal,
      const std::vector<double>& coords, const double& time) {
    ...
    for (int ipoint = 0; ipoint < num_points; ipoint++) {
      ...
      bdry_u[ind++] = rho_ * wf_;
      bdry_u[ind] =
          rm1_inv_ * p + 0.5 * rho_ * (uf_ * uf_ + vf_ * vf_ + wf_ * wf_);
    }
  }
  void SupersonicInflowBdryEuler3D::ComputeBdryFlux(
      const int num_points, std::vector<double>& flux, FACE_INPUTS,
      const std::vector<double>& coords, const double& time) {
    const double pb = p_over_pinf_ * r_inv_;
    const double dEb =
        pb * rm1_inv_ + 0.5 * rho_ * (uf_ * uf_ + vf_ * vf_ + wf_ * wf_);
    int ind = 0;
    for (int ipoint = 0; ipoint < num_points; ipoint++) {
      // pds
      ind = DS_ * ipoint;
      ...
      flux[ind++] = rho_ * uf_ * wf_;
      flux[ind++] = (dEb + pb) * uf_;

      flux[ind++] = rho_ * vf_;
      flux[ind++] = rho_ * vf_ * uf_;
      flux[ind++] = rho_ * vf_ * vf_ + pb;
      flux[ind++] = rho_ * vf_ * wf_;
      flux[ind++] = (dEb + pb) * vf_;

      flux[ind++] = rho_ * wf_;
      flux[ind++] = rho_ * wf_ * uf_;
      flux[ind++] = rho_ * wf_ * vf_;
      flux[ind++] = rho_ * wf_ * wf_ + pb;
      flux[ind] = (dEb + pb) * wf_;
    }
  }
  void SupersonicInflowBdryEuler3D::ComputeBdrySolutionJacobi(
      const int num_points, double* bdry_u_jacobi,
      const std::vector<double>& owner_u,
      const std::vector<double>& owner_div_u, const std::vector<double>& normal,
      const std::vector<double>& coords, const double& time) {
  ...
  void SupersonicInflowBdryEuler3D::ComputeBdryFluxJacobi(
      const int num_points, std::vector<double>& flux_jacobi,
      std::vector<double>& flux_grad_jacobi, FACE_INPUTS,
      const std::vector<double>& coords, const double& time) {
    static const int max_num_bdry_points = equation_->GetMaxNumBdryPoints();
  ...
  BackPressureBdryEuler3D::BackPressureBdryEuler3D(const int bdry_tag,
                                                   EquationEuler3D* equation)
      : BoundaryEuler3D(bdry_tag, equation) {
  ...
  void BackPressureBdryEuler3D::ComputeBdrySolution(
      const int num_points, std::vector<double>& bdry_u,
      std::vector<double>& bdry_div_u, const std::vector<double>& owner_u,
      const std::vector<double>& owner_div_u, const std::vector<double>& normal,
      const std::vector<double>& coords, const double& time) {
    ...
    for (int ipoint = 0; ipoint < num_points; ipoint++) {
      ...
      // ps
      GET_SOLUTION_PS(, owner_u);
      const double p = rm1_ * (dE - 0.5 * (du * du + dv * dv + dw * dw) / d);
  ...
  void BackPressureBdryEuler3D::ComputeBdrySolutionJacobi(
      const int num_points, double* bdry_u_jacobi,
      const std::vector<double>& owner_u,
      const std::vector<double>& owner_div_u, const std::vector<double>& normal,
      const std::vector<double>& coords, const double& time) {
    ...
    for (int ipoint = 0; ipoint < num_points; ipoint++) {
      ...
      const auto p = rm1_ * (dE - 0.5 * (du * du + dv * dv + dw * dw) / d);
      const auto a = std::sqrt(r_ * p / d);
      const auto V = (du * nx + dv * ny + dw * nz) / d;
      ...
      if (std::abs(local_M) >= 1.0) {  // supersonic
        ...
        if (local_M >= 0.0) {
          ...
          const auto wb = std::abs(V) * nz;

          bdry_u[0] = p_ratio * d;
          bdry_u[1] = p_ratio * d * ub;
          bdry_u[2] = p_ratio * d * vb;
          bdry_u[3] = p_ratio * d * wb;
          bdry_u[4] = rm1_inv_ * pb + 0.5 * p_ratio * d * V * V;
        }
      }
  ...
  void BackPressureBdryEuler3D::ComputeBdryFluxJacobi(
      const int num_points, std::vector<double>& flux_jacobi,
      std::vector<double>& flux_grad_jacobi, FACE_INPUTS,
      const std::vector<double>& coords, const double& time) {
    static const int max_num_bdry_points = equation_->GetMaxNumBdryPoints();
  ...
  // -------------------------------- Problem --------------------------------
  // //
  std::shared_ptr<ProblemEuler3D> ProblemEuler3D::GetProblem(
      const std::string& name) {
    if (!name.compare("Constant"))
      return std::make_shared<ConstantEuler3D>();
    else if (!name.compare("FreeStream"))
      return std::make_shared<FreeStreamEuler3D>();
    else if (!name.compare("DoubleSine"))
      return std::make_shared<DoubleSineEuler3D>();
    else if (!name.compare("ShockTube"))
      return std::make_shared<ShockTubeEuler3D>();
    else
  ...
  ConstantEuler3D::ConstantEuler3D() {
  ...
  void ConstantEuler3D::Problem(
      const int num_points, std::vector<double>& solutions,
      const std::vector<double>& coord, const double time) const {
    int ind = 0;
    for (int ipoint = 0; ipoint < num_points; ipoint++)
      for (int istate = 0; istate < S_; istate++)
  ...
  FreeStreamEuler3D::FreeStreamEuler3D() {
    ...
    str << "\t\tSideslip angle = " << sideslip_ << " (deg)\n";
    MASTER_MESSAGE(str.str());
  }
  void FreeStreamEuler3D::Problem(
      const int num_points, std::vector<double>& solutions,
      const std::vector<double>& coord, const double time) const {
    int ind = 0;
    for (int ipoint = 0; ipoint < num_points; ipoint++) {
  ...
  // Problem = DoubleSine
  // ProblemInput = -
  DoubleSineEuler3D::DoubleSineEuler3D()
  ...
  void DoubleSineEuler3D::Problem(
      const int num_points, std::vector<double>& solutions,
      const std::vector<double>& coord, const double time) const {
    int ind = 0;
    std::vector<double> values(S_);
    ...
    for (int ipoint = 0; ipoint < num_points; ipoint++) {
      ...
      values[0] = 1.0 + 0.2 *
                            std::sin(2.0 * pi_ * wave_number_[0] *
                                     (x - velocity_[0] * time)) *
                            std::sin(2.0 * pi_ * wave_number_[1] *
                                     (y - velocity_[1] * time)) *
                            std::sin(2.0 * pi_ * wave_number_[2] *
                                     (z - velocity_[2] * time));
  ...
  ShockTubeEuler3D::ShockTubeEuler3D() {
    ...
    right_values_[0] =
        std::stod(config->GetConfigValue(PROBLEM_INPUT_I(ind++)));
    right_values_[1] =
        std::stod(config->GetConfigValue(PROBLEM_INPUT_I(ind++)));
    right_values_[2] = 0.0;
    right_values_[3] = 0.0;
