...
#define GET_SOLUTION_PS(tag, data)   \
  ...
  ind += ns_;                        \
  const auto& du##tag = data[ind++]; \
  const auto& dv##tag = data[ind++]; \
  const auto& dE##tag = data[ind++]; \
  const auto& de_eev##tag = data[ind]
...
#define GET_PRI_SOLUTION_PS(tag, data)                                \
  ind = S_ * ipoint;                                                  \
  const auto* d##tag = &data[ind];                                    \
  ind += ns_;                                                         \
...
#define GET_PRI_SOLUTION_GRAD_PDS(tag, data) \
  ind = DS_ * ipoint;                        \
...
namespace deneb {
// ------------------------------- Constants ------------------------------- //
int ConstantsNS2DNeq2Tndcs::S_ = 0;
int ConstantsNS2DNeq2Tndcs::DS_ = 0;
int ConstantsNS2DNeq2Tndcs::SS_ = 0;
int ConstantsNS2DNeq2Tndcs::DSS_ = 0;
int ConstantsNS2DNeq2Tndcs::DDSS_ = 0;
int ConstantsNS2DNeq2Tndcs::ax_ = 0;
std::shared_ptr<Neq2T::Mixture> ConstantsNS2DNeq2Tndcs::mixture_ = NULL;
int ConstantsNS2DNeq2Tndcs::ns_ = 0;
double ConstantsNS2DNeq2Tndcs::T_eev_min_ = 0.0;
double ConstantsNS2DNeq2Tndcs::L_ref_ = 0.0;
double ConstantsNS2DNeq2Tndcs::rho_ref_ = 0.0;
double ConstantsNS2DNeq2Tndcs::v_ref_ = 0.0;
double ConstantsNS2DNeq2Tndcs::e_ref_ = 0.0;
double ConstantsNS2DNeq2Tndcs::T_ref_ = 0.0;
double ConstantsNS2DNeq2Tndcs::p_ref_ = 0.0;
double ConstantsNS2DNeq2Tndcs::D_ref_ = 0.0;
double ConstantsNS2DNeq2Tndcs::mu_ref_ = 0.0;
double ConstantsNS2DNeq2Tndcs::k_ref_ = 0.0;

ConstantsNS2DNeq2Tndcs::ConstantsNS2DNeq2Tndcs() {
  if (mixture_ == NULL) {
...
void ConstantsNS2DNeq2Tndcs::sol2pri(const int num_points, const double* sol,
                                     double* pri) const {
  std::vector<double> dim_rho(ns_, 0.0);
  for (int ipoint = 0; ipoint < num_points; ipoint++) {
    const double* single_sol = &sol[ipoint * S_];
    const double* rho = &single_sol[0];
    const double& du = single_sol[ns_];
    const double& dv = single_sol[ns_ + 1];
    const double& dE = single_sol[ns_ + 2];
    const double& de_eev = single_sol[ns_ + 3];
        
    for (int ispecies = 0; ispecies < ns_; ispecies++)
      dim_rho[ispecies] = rho[ispecies] * rho_ref_;
    ...
    const auto e = dE * d_inv - 0.5 * (u * u + v * v);
    const auto e_eev = de_eev * d_inv;

    const auto T_tr =
        mixture_->GetTransRotationTemperature(e * e_ref_, e_eev * e_ref_) /
        T_ref_;
    const auto T_eev =
        mixture_->GetElectronicVibrationTemperature(e_eev * e_ref_) / T_ref_;

    double* single_pri = &pri[ipoint * S_];
    for (int i = 0; i < ns_; i++) single_pri[i] = rho[i];
    single_pri[ns_] = u;
    single_pri[ns_ + 1] = v;
    single_pri[ns_ + 2] = T_tr;
    single_pri[ns_ + 3] = T_eev;
  }
}
void ConstantsNS2DNeq2Tndcs::pri2sol(const int num_points, const double* pri,
                                     double* sol) const {
  std::vector<double> dim_rho(ns_, 0.0);
  for (int ipoint = 0; ipoint < num_points; ipoint++) {
    const double* single_pri = &pri[ipoint * S_];
    const double* rho = &single_pri[0];
    const double& u = single_pri[ns_];
    const double& v = single_pri[ns_ + 1];
    const double& T_tr = single_pri[ns_ + 2];
    const double& T_eev = single_pri[ns_ + 3];
    ...
    double* single_sol = &sol[ipoint * S_];
    for (int i = 0; i < ns_; i++) single_sol[i] = rho[i];
    single_sol[ns_] = du;
    single_sol[ns_ + 1] = dv;
    single_sol[ns_ + 2] = dE;
    single_sol[ns_ + 3] = de_eev;
  }
}
void ConstantsNS2DNeq2Tndcs::gradsol2gradpri(const int num_points,
                                             const double* pri_jacobi,
                                             const double* grad_sol,
                                             double* grad_pri) const {
    for (int ipoint = 0; ipoint < num_points; ipoint++) {
    gemvAx(1.0, &pri_jacobi[ipoint * SS_], S_, &grad_sol[ipoint * S_ * 2], 1,
           0.0, &grad_pri[ipoint * S_ * 2], 1, S_, S_);
    gemvAx(1.0, &pri_jacobi[ipoint * SS_], S_, &grad_sol[ipoint * S_ * 2 + S_],
           1, 0.0, &grad_pri[ipoint * S_ * 2 + S_], 1, S_, S_);
  }
}
void ConstantsNS2DNeq2Tndcs::PriJacobian(const int num_points,
                                         const double* pri,
                                         double* pri_jacobi) const
{
  // dW/dU where W: primitive, U: conservative
  static std::vector<double> dim_rho(ns_, 0.0);
  double* local_pri_jacobi;

  memset(pri_jacobi, 0, SS_ * sizeof(double) * num_points);
  ...
  for (int ipoint = 0; ipoint < num_points; ipoint++) {
    ...
    local_pri_jacobi = &pri_jacobi[ipoint * SS_];

    for (int ispec = 0; ispec < ns_; ispec++)
      dim_rho[ispec] = rho[ispec] * rho_ref_;
    ...
    double dCv_eev = 0.0;
    for (int i = 0; i < ns_; i++) {
      const double Cv_tr = species[i]->GetTransRotationSpecificHeat(dim_T_tr) * T_ref_ / e_ref_;
      const double Cv_eev =
          species[i]->GetElectronicVibrationSpecificHeat(dim_T_eev) * T_ref_ /
          e_ref_;
      dCv_tr += rho[i] * Cv_tr;
      dCv_eev += rho[i] * Cv_eev;

      local_pri_jacobi[i * S_ + i] = 1.0;
      local_pri_jacobi[ns_ * S_ + i] = -u / d;
      local_pri_jacobi[(ns_ + 1) * S_ + i] = -v / d;
    }
    local_pri_jacobi[ns_ * S_ + ns_] = 1.0 / d;
    local_pri_jacobi[(ns_ + 1) * S_ + (ns_ + 1)] = 1.0 / d;

    const double dCv_tr_inv = 1.0 / dCv_tr;
    const double dCv_eev_inv = 1.0 / dCv_eev;
    const double hV2 = 0.5 * (u * u + v * v);
    for (int i = 0; i < ns_; i++) {
      ...
      local_pri_jacobi[(ns_ + 2) * S_ + i] = (hV2 - e + e_eev) * dCv_tr_inv;
      local_pri_jacobi[(ns_ + 3) * S_ + i] = -e_eev * dCv_eev_inv;
    }
    local_pri_jacobi[(ns_ + 2) * S_ + ns_] = -u * dCv_tr_inv;
    local_pri_jacobi[(ns_ + 2) * S_ + (ns_ + 1)] = -v * dCv_tr_inv;
    local_pri_jacobi[(ns_ + 2) * S_ + (ns_ + 2)] = dCv_tr_inv;
    local_pri_jacobi[(ns_ + 2) * S_ + (ns_ + 3)] = -dCv_tr_inv;
    local_pri_jacobi[(ns_ + 3) * S_ + (ns_ + 3)] = dCv_eev_inv;
  }
}
void ConstantsNS2DNeq2Tndcs::ConsJacobian(const int num_points,
  const double* pri_sol, double* sol_jacobi) const
{
  const auto& species = mixture_->GetSpecies();
  std::vector<aDual> values(S_, aDual(S_));
  std::vector<double> dim_d(ns_);
  for (int ipoint = 0; ipoint < num_points; ipoint++) {
    const double* sol = &pri_sol[ipoint * S_];
    ...
    int ind = SS_ * ipoint;
    for (int i = 0; i < S_; i++)
      for (int j = 0; j < S_; j++) sol_jacobi[ind++] = values[i].df[j];
  }
}
    // ------------------------------- Equation -------------------------------- //
EquationNS2DNeq2Tndcs::EquationNS2DNeq2Tndcs(bool axis)
    : ConstantsNS2DNeq2Tndcs(), Equation(D_, S_, true) {
  if (axis) {
    ax_ = 1;
    MASTER_MESSAGE(avocado::GetTitle("EquationNS2DNeq2Tndcs (Axi-symmetric)"));
    ERROR_MESSAGE("Axi-symmetric is not supported in this version!!!");
  } else {
    MASTER_MESSAGE(avocado::GetTitle("EquationNS2DNeq2Tndcs"));
  }
  num_species_ = ns_;
  ...
  auto& config = AVOCADO_CONFIG;
  problem_ = ProblemNS2DNeq2Tndcs::GetProblem(config->GetConfigValue(PROBLEM));
  const std::string& numflux = config->GetConfigValue(CONVECTIVE_FLUX);
  if (!numflux.compare("LLF")) {
    ASSIGN_FLUX(EquationNS2DNeq2Tndcs, LLF);
  } else
...
EquationNS2DNeq2Tndcs::~EquationNS2DNeq2Tndcs() {
  problem_.reset();
  boundaries_.clear();
  boundary_registry_.clear();
}
void EquationNS2DNeq2Tndcs::RegistBoundary(const std::vector<int>& bdry_tag) {
  auto& config = AVOCADO_CONFIG;
  ...
  for (auto&& tag : all_bdry_tag) {
    ...
    if (boundary_registry_.find(tag) == boundary_registry_.end())
      boundary_registry_[tag] =
          BoundaryNS2DNeq2Tndcs::GetBoundary(bdry_type, tag, this);
  }
}
void EquationNS2DNeq2Tndcs::BuildData(void) {
  MASTER_MESSAGE(avocado::GetTitle("EquationNS2DNeq2Tndcs::BuildData()"));
  ...
  variable_names.push_back("rhoU");
  variable_names.push_back("rhoV");
  variable_names.push_back("rhoE");
  variable_names.push_back("rho*e_eev");

  cell_variable_names_ = variable_names;
  cell_variable_names_.push_back("T_tr");
  cell_variable_names_.push_back("T_eev");
  cell_variable_names_.push_back("rho");
  cell_variable_names_.push_back("U");
  cell_variable_names_.push_back("V");
...
void EquationNS2DNeq2Tndcs::GetCellPostSolution(
  ...
  for (int ipoint = 0; ipoint < num_points; ipoint++) {
    ...
    const double du = sol[ns_] * rho_ref_ * v_ref_;
    const double dv = sol[ns_ + 1] * rho_ref_ * v_ref_;
    const double dE = sol[ns_ + 2] * rho_ref_ * e_ref_;
    const double de_eev = sol[ns_ + 3] * rho_ref_ * e_ref_;
    post_solution[ind++] = du;
    post_solution[ind++] = dv;
    post_solution[ind++] = dE;
    post_solution[ind++] = de_eev;
    ...
    const double e = dE * d_inv - 0.5 * (u * u + v * v);
    const double e_eev = de_eev * d_inv;    

    const double T_tr = mixture_->GetTransRotationTemperature(e, e_eev);
    const double T_eev = mixture_->GetElectronicVibrationTemperature(e_eev);
...
void EquationNS2DNeq2Tndcs::GetFacePostSolution(
...
void EquationNS2DNeq2Tndcs::ComputeInitialSolution(double* solution,
                                                   const double t) {
  const int& order = DENEB_DATA->GetOrder();
  const int& num_bases = DENEB_DATA->GetNumBases();
  const int& num_cells = DENEB_DATA->GetNumCells();
...
void EquationNS2DNeq2Tndcs::ComputeLocalTimestep(
  ...
  for (int icell = 0; icell < num_cells; icell++) {
    ...
    const double& du = sol[ns_ * num_bases] * cell_basis_value[icell][0];
    const double& dv = sol[(ns_ + 1) * num_bases] * cell_basis_value[icell][0];
    const double& dE =
    ...
    const auto d_inv = 1.0 / mixture_d;
    const auto u = du * d_inv;
    const auto v = dv * d_inv;
    const auto e = dE * d_inv - 0.5 * (u * u + v * v);
    const auto e_eev = de_eev * d_inv;
    ...
    const auto dim_T_tr =
        mixture_->GetTransRotationTemperature(e * e_ref_, e_eev * e_ref_);
    const auto dim_T_eev =
        mixture_->GetElectronicVibrationTemperature(e_eev * e_ref_);
    const auto T_tr = dim_T_tr / T_ref_;
    const auto T_eev = dim_T_eev / T_ref_;
...
double EquationNS2DNeq2Tndcs::ComputeMaxCharacteristicSpeed(
    const double* input_solution) const {
  const double* rho = input_solution;
  const double& du = input_solution[ns_];
  const double& dv = input_solution[ns_ + 1];
  const double& dE = input_solution[ns_ + 2];
  const double& de_eev = input_solution[ns_ + 3];
  ...
  const double beta = mixture_->GetBeta(dim_T_tr);
  const double p = mixture_->GetPressure(dim_T_tr, dim_T_eev) / p_ref_;
  const double a = std::sqrt((1.0 + beta) * p / mixture_d);
  const double V = std::sqrt(u * u + v * v);

  return V + a;
}
void EquationNS2DNeq2Tndcs::ComputeRHS(const double* solution, double* rhs,
                                       const double t) {
  static const int& num_bases = DENEB_DATA->GetNumBases();
  static const int& num_cells = DENEB_DATA->GetNumCells();
  static const int sb = S_ * num_bases;
  ...
  for (int ibdry = 0; ibdry < num_bdries; ibdry++) {
    ...
    avocado::Kernel1::f59(&bdry_owner_coefficients[ibdry][0],
                          &solution_difference[0], &local_auxiliary[0], D_,
                          num_bases, num_points, S_, 0.5, 0.0); // for debug
...
void EquationNS2DNeq2Tndcs::ComputeSystemMatrix(const double* solution,
  ...
  for (int ibdry = 0; ibdry < num_bdries; ibdry++) {
    ...
    //avocado::Kernel1::f59(&bdry_owner_coefficients[ibdry][0],
    //                      &solution_difference[0], &local_auxiliary[0], D_,
    //                      num_bases, num_points, S_, 1.0, 0.0); // for debug
    avocado::Kernel1::f59(&bdry_owner_coefficients[ibdry][0],
                          &solution_difference[0], &local_auxiliary[0], D_,
                          num_bases, num_points, S_, 0.5, 0.0);
    avocado::Kernel1::f42(&bdry_owner_basis_grad_value[ibdry][0],
...
void EquationNS2DNeq2Tndcs::SolutionLimit(double* solution)
{
  static const int& num_bases = DENEB_DATA->GetNumBases();
  static const int& num_cells = DENEB_DATA->GetNumCells();
  static const int sb = S_ * num_bases;
  ...
  static std::vector<double> owner_w(S_ * max_num_points_);
  static std::vector<double> neighbor_w(S_ * max_num_points_);
  static const int& num_inner_faces = DENEB_DATA->GetNumInnerFaces();
  static const auto& num_face_points = DENEB_DATA->GetNumFacePoints();
  static const auto& face_owner_cell = DENEB_DATA->GetFaceOwnerCell();
  static const auto& face_neighbor_cell = DENEB_DATA->GetFaceNeighborCell();
  ...
  for (int iface = 0; iface < num_inner_faces; iface++) {
    ...
    sol2pri(num_points, &owner_u[0], &owner_w[0]);
    sol2pri(num_points, &neighbor_u[0], &neighbor_w[0]);
    for (int ipoint = 0; ipoint < num_points; ipoint++) {
      ind = S_ * ipoint;                   
      const auto* d_o = &owner_w[ind];
      const auto& T_tr_o = owner_w[ind + ns_ + 2];
      const auto& T_eev_o = owner_w[ind + ns_ + 3];                                 

      const auto* d_n = &neighbor_w[ind];
      const auto& T_tr_n = neighbor_w[ind + ns_ + 2];
      const auto& T_eev_n = neighbor_w[ind + ns_ + 3];                                 
  ...
  for (int iface = num_inner_faces; iface < num_faces; iface++) {
    ...
    for (int ipoint = 0; ipoint < num_points; ipoint++) {
      ind = S_ * ipoint;
      const auto* d = &owner_w[ind];
      const auto& T_tr = owner_w[ind + ns_ + 2];
      const auto& T_eev = owner_w[ind + ns_ + 3];
  ...
  for (int ibdry = 0; ibdry < num_bdries; ibdry++) {
    ...
    avocado::Kernel0::f4(&solution[owner_cell * sb],
                         &bdry_owner_basis_value[ibdry][0], &owner_u[0],
                         S_, num_bases, num_points, 1.0, 0.0);
  ...
  for (int icell = 0; icell < num_cells; icell++) {
    ...
    avocado::Kernel0::f4(&solution[icell * sb], &cell_basis_value[icell][0],
                         &owner_u[0], S_, num_bases, num_points, 1.0,
                         0.0);
    ...
    sol0[ns_] = sol[ns_ * num_bases] * cell_basis_value[icell][0];
    sol0[ns_ + 1] = sol[(ns_ + 1) * num_bases] * cell_basis_value[icell][0];
    sol0[ns_ + 2] = sol[(ns_ + 2) * num_bases] * cell_basis_value[icell][0];
    sol0[ns_ + 3] = sol[(ns_ + 3) * num_bases] * cell_basis_value[icell][0];

    bool limit_flag_Ttr = false;
    bool limit_flag_Teev = false;
    sol2pri(1, &sol0[0], &pri0[0]);
    if (pri0[ns_ + 2] < 10 / T_ref_) {
      pri0[ns_ + 2] = 10 / T_ref_;
      limit_flag_Ttr = true;
    }
    if (pri0[ns_ + 3] < 10 / T_ref_) {
      pri0[ns_ + 3] = 10 / T_ref_;
      limit_flag_Teev = true;
    }
    pri2sol(1, &pri0[0], &sol0[0]);
...
void EquationNS2DNeq2Tndcs::ComputeComFlux(
  ...
  std::vector<double> owner_w(num_points * S_, 0.0);
  std::vector<double> owner_div_w(num_points * S_ * 2, 0.0);
  std::vector<double> pri_jacobi(num_points * SS_, 0.0);
  ...
  std::vector<double> dim_d(ns_);
  sol2pri(num_points, &owner_u[0], &owner_w[0]);
  PriJacobian(num_points, &owner_w[0], &pri_jacobi[0]);
  gradsol2gradpri(num_points, &pri_jacobi[0], &owner_div_u[0], &owner_div_w[0]);

  int ind = 0;
  double arvis = 0.0;
  for (int ipoint = 0; ipoint < num_points; ipoint++) {
    GET_PRI_SOLUTION_PS(, owner_w);
    GET_SOLUTION_GRAD_PDS(, owner_div_u);
    GET_PRI_SOLUTION_GRAD_PDS(, owner_div_w);
    arvis =
...
void EquationNS2DNeq2Tndcs::ComputeComFluxJacobi(
  ...
  std::vector<double> owner_div_w(num_points * S_ * 2, 0.0);
  std::vector<double> pri_jacobi(num_points * SS_, 0.0);
  static std::vector<double> flux_pri_jacobi(DSS_, 0.0);
  static std::vector<double> flux_pri_grad_jacobi(DDSS_, 0.0);
  ...
  for (int ipoint = 0; ipoint < num_points; ipoint++) {
    {
      ...
      std::vector<aDual> d(ns_, aDual(S_));
      for (int i = 0; i < ns_; i++) {
        dim_d[i] = owner_w[ind] * rho_ref_;
        d[i] = aDual(S_, owner_w[ind++], i);
      }
      aDual u(S_, owner_w[ind++], ns_);
      aDual v(S_, owner_w[ind++], ns_ + 1);
      const double& T_tr = owner_w[ind++];
      const double& T_eev = owner_w[ind];
      const double dim_T_tr = T_tr * T_ref_;
      const double dim_T_eev = T_eev * T_ref_;
      ...
      for (int ds = 0; ds < DS_; ds++)
        for (int istate = 0; istate < S_; istate++)
          flux_pri_jacobi[ind++] = flux1[ds].df[istate];

      gemmAB(1.0, &flux_pri_jacobi[0], &pri_jacobi[ipoint * SS_], 0.0,
             &flux_jacobi[DSS_ * ipoint], S_, S_, S_);
      gemmAB(1.0, &flux_pri_jacobi[SS_], &pri_jacobi[ipoint * SS_], 0.0,
             &flux_jacobi[DSS_ * ipoint + SS_], S_, S_, S_);
    }
    {
      GET_PRI_SOLUTION_PS(, owner_w);
      // pds over psd
      ...
      for (int i = 0; i < ns_; i++)
        dx[i] = aDual(DS_, owner_div_w[ind++], 2 * i);
      const aDual ux(DS_, owner_div_w[ind++], 2 * ns_);
      const aDual vx(DS_, owner_div_w[ind++], 2 * (ns_ + 1));
      const aDual Ttrx(DS_, owner_div_w[ind++], 2 * (ns_ + 2));
      const aDual Teevx(DS_, owner_div_w[ind++], 2 * (ns_ + 3));
      std::vector<aDual> dy(ns_, aDual(DS_));
      for (int i = 0; i < ns_; i++)
        dy[i] = aDual(DS_, owner_div_w[ind++], 2 * i + 1);
      const aDual uy(DS_, owner_div_w[ind++], 2 * ns_ + 1);
      const aDual vy(DS_, owner_div_w[ind++], 2 * (ns_ + 1) + 1);
      const aDual Ttry(DS_, owner_div_w[ind++], 2 * (ns_ + 2) + 1);
      const aDual Teevy(DS_, owner_div_w[ind], 2 * (ns_ + 3) + 1);
      ...
      for (int ds = 0; ds < DS_; ds++)
        for (int sd = 0; sd < DS_; sd++)
          flux_pri_grad_jacobi[ind++] = flux2[ds].df[sd];

      for (int istate = 0; istate < S_; istate++)
        for (int idim = 0; idim < D_; idim++)
          for (int jstate = 0; jstate < S_; jstate++)
            for (int jdim = 0; jdim < D_; jdim++) {
              const int irow = idim * S_ + istate;
              const int icolumn = jstate * D_ + jdim;
              double sum = 0.0;
              
              for (int kstate = 0; kstate < S_; kstate++)
                sum += flux_pri_grad_jacobi[irow * DS_ + kstate * D_ + jdim] *
                       pri_jacobi[ipoint * SS_ + kstate * S_ + jstate];

              flux_grad_jacobi[DDSS_ * ipoint + irow * DS_ + icolumn] = sum;
            }
      for (int istate = 0; istate < S_; istate++)
        for (int idim = 0; idim < D_; idim++) {
          const int irow = idim * S_ + istate;
          const int icolumn = istate * D_ + idim;
          flux_grad_jacobi[DDSS_ * ipoint + irow * DS_ + icolumn] -= arvis;
        }
    }
  }
}
void EquationNS2DNeq2Tndcs::ComputeSource(
  ...
  for (int ipoint = 0; ipoint < num_points; ipoint++) {
    ...
    mixture_->SetDensity(&dim_d[0]);
     ...
     for (int i = 0; i < ns_; i++) source[ind++] = rate[i] * L_ref_ * inv_rhov;
    ...
    source[ind++] = 0.0;
    source[ind++] = 0.0;
    source[ind] = (ct + vt + et) * L_ref_ * inv_rhoev;
  }
}
void EquationNS2DNeq2Tndcs::ComputeSourceJacobi(
  ...
  static std::vector<double> source_pri_jacobi(SS_, 0.0);
  std::vector<double> owner_w(num_points * S_, 0.0);
  std::vector<double> pri_jacobi(num_points * SS_, 0.0);
  ...
  for (int ipoint = 0; ipoint < num_points; ipoint++) {
    ...
    gemmAB(1.0, &source_pri_jacobi[0], &pri_jacobi[ipoint * SS_], 0.0,
      &source_jacobi[SS_ * ipoint], S_, S_, S_);

    // Where is ett?
  }
}
void EquationNS2DNeq2Tndcs::ComputeSolutionJacobi(
  ...
  sol2pri(num_points, &owner_u[0], &owner_w[0]);
  for (int ipoint = 0; ipoint < num_points; ipoint++) {
    const double* sol = &owner_w[ipoint * S_];
    std::vector<aDual> d(ns_, aDual(S_));
    for (int i = 0; i < ns_; i++) {
      dim_d[i] = sol[i] * rho_ref_;
      d[i] = aDual(S_, sol[i], i);
...
void EquationNS2DNeq2Tndcs::ComputeNumFluxLLF(const int num_points,
  ...
  std::vector<double> owner_w(num_points * S_, 0.0);
  std::vector<double> neighbor_w(num_points * S_, 0.0);
  std::vector<double> owner_div_w(num_points * S_ * 2, 0.0);
  std::vector<double> neighbor_div_w(num_points * S_ * 2, 0.0);
  std::vector<double> owner_pri_jacobi(num_points * SS_, 0.0);
  std::vector<double> neighbor_pri_jacobi(num_points * SS_, 0.0);
  ...
  sol2pri(num_points, &owner_u[0], &owner_w[0]);
  sol2pri(num_points, &neighbor_u[0], &neighbor_w[0]);
  PriJacobian(num_points, &owner_w[0], &owner_pri_jacobi[0]);
  PriJacobian(num_points, &neighbor_w[0], &neighbor_pri_jacobi[0]);
  gradsol2gradpri(num_points, &owner_pri_jacobi[0], &owner_div_u[0],
                  &owner_div_w[0]);
  gradsol2gradpri(num_points, &neighbor_pri_jacobi[0], &neighbor_div_u[0],
                  &neighbor_div_w[0]);

  for (int ipoint = 0; ipoint < num_points; ipoint++) {
    GET_NORMAL_PD(normal);

    GET_PRI_SOLUTION_PS(_o, owner_w);
    GET_PRI_SOLUTION_PS(_n, neighbor_w);

    GET_SOLUTION_GRAD_PDS(_o, owner_div_u);
    GET_SOLUTION_GRAD_PDS(_n, neighbor_div_u);

    GET_PRI_SOLUTION_GRAD_PDS(_o, owner_div_w);
    GET_PRI_SOLUTION_GRAD_PDS(_n, neighbor_div_w);
    ...
    const double k_eev_o =
        mixture_->GetElectronicVibrationConductivity(dim_T_tr_o, dim_T_eev_o) /
        k_ref_;
    ...
    // Viscous flux
    const double mu_n =
        mixture_->GetViscosity(dim_T_tr_n, dim_T_eev_n) / mu_ref_;
    const double k_tr_n =
        mixture_->GetTransRotationConductivity(dim_T_tr_n, dim_T_eev_n) /
        k_ref_;
    const double k_eev_n =
        mixture_->GetElectronicVibrationConductivity(dim_T_tr_n, dim_T_eev_n) /
        k_ref_;
...
void EquationNS2DNeq2Tndcs::ComputeNumFluxJacobiLLF(const int num_points,
                                                    FACE_JACOBI_OUTPUTS,
                                                    FACE_INPUTS) {
  // flux_jacobi(ds1s2) = F(ds1) over U(s2)
  // flux_grad_jacobi(d1s1 s2d2) = F(d1s1) over gradU(d2s2)
  ...
  for (int ipoint = 0; ipoint < num_points; ipoint++) {
        ...
        neighbor_cell, ipoint);
    {
      ...
      const double k_tr_n =
          mixture_->GetTransRotationConductivity(dim_T_tr_n, dim_T_eev_n) /
          k_ref_;
      const double k_eev_n = mixture_->GetElectronicVibrationConductivity(
                                 dim_T_tr_n, dim_T_eev_n) /
                             k_ref_;
      ...
      const double q_eev_y_n = k_eev_n * Teevy_n;
      {
        ...
        for (int i = 0; i < ns_; i++) {
          dim_d_o[i] = owner_w[ind] * rho_ref_;
          d_o[i] = aDual(S_, owner_w[ind++], i);
        }
        aDual u_o(S_, owner_w[ind++], ns_);
        aDual v_o(S_, owner_w[ind++], ns_ + 1);
        const double& T_tr_o = owner_w[ind++];
        const double& T_eev_o = owner_w[ind];
        ...
        gemmAB(1.0, &flux_pri_jacobi[0], &owner_pri_jacobi[ipoint * SS_], 0.0,
               &flux_owner_jacobi[DSS_ * ipoint], S_, S_, S_);
        gemmAB(1.0, &flux_pri_jacobi[SS_], &owner_pri_jacobi[ipoint * SS_], 0.0,
               &flux_owner_jacobi[DSS_ * ipoint + SS_], S_, S_, S_);
      }
      {
        ...
        for (int i = 0; i < ns_; i++)
          dx_o[i] = aDual(DS_, owner_div_w[ind++], 2 * i);
        const aDual ux_o(DS_, owner_div_w[ind++], 2 * ns_);
        const aDual vx_o(DS_, owner_div_w[ind++], 2 * (ns_ + 1));
        const aDual Ttrx_o(DS_, owner_div_w[ind++], 2 * (ns_ + 2));
        const aDual Teevx_o(DS_, owner_div_w[ind++], 2 * (ns_ + 3));
        std::vector<aDual> dy_o(ns_, aDual(DS_));
        for (int i = 0; i < ns_; i++)
          dy_o[i] = aDual(DS_, owner_div_w[ind++], 2 * i + 1);
        const aDual uy_o(DS_, owner_div_w[ind++], 2 * ns_ + 1);
        const aDual vy_o(DS_, owner_div_w[ind++], 2 * (ns_ + 1) + 1);
        const aDual Ttry_o(DS_, owner_div_w[ind++], 2 * (ns_ + 2) + 1);
        const aDual Teevy_o(DS_, owner_div_w[ind], 2 * (ns_ + 3) + 1);
          ...
          for (int idim = 0; idim < D_; idim++)
            for (int jstate = 0; jstate < S_; jstate++)
              for (int jdim = 0; jdim < D_; jdim++) {
                ...
                for (int kstate = 0; kstate < S_; kstate++)
                  sum += flux_pri_grad_jacobi[irow * DS_ + kstate * D_ + jdim] *
                         owner_pri_jacobi[ipoint * SS_ + kstate * S_ + jstate];
      ...
      }
      {
        ...
        const double beta_o = mixture_->GetBeta(dim_T_tr_o);
        const double a_o =
            std::sqrt((1.0 + beta_o) * mixture_p_o / mixture_d_o);
        const double V_o = u_o * nx + v_o * ny;
        double de_o = 0.0;
        double de_eev_o = 0.0;
        for (int i = 0; i < ns_; i++) {
        ...
        const double k_tr_o =
            mixture_->GetTransRotationConductivity(dim_T_tr_o, dim_T_eev_o) /
            k_ref_;
        const double k_eev_o = mixture_->GetElectronicVibrationConductivity(
                                   dim_T_tr_o, dim_T_eev_o) /
                               k_ref_;
        ...
        const double q_eev_y_o = k_eev_o * Teevy_o;
        {
          ...
          for (int i = 0; i < ns_; i++) {
            dim_d_n[i] = neighbor_w[ind] * rho_ref_;
            d_n[i] = aDual(S_, neighbor_w[ind++], i);
          }
          aDual u_n(S_, neighbor_w[ind++], ns_);
          aDual v_n(S_, neighbor_w[ind++], ns_ + 1);
          const double& T_tr_n = neighbor_w[ind++];
          const double& T_eev_n = neighbor_w[ind];
          ...
          mixture_p_n.f = mixture_->GetPressureJacobian(dim_T_tr_n, dim_T_eev_n,
                                                        &mixture_p_n.df[0]);
          ...
          const auto a_n =
              std::sqrt((1.0 + beta_n) * mixture_p_n / mixture_d_n);
          ...
          gemmAB(1.0, &flux_pri_jacobi[0], &neighbor_pri_jacobi[ipoint * SS_],
                 0.0, &flux_neighbor_jacobi[DSS_ * ipoint], S_, S_, S_);
          gemmAB(1.0, &flux_pri_jacobi[SS_], &neighbor_pri_jacobi[ipoint * SS_],
                 0.0, &flux_neighbor_jacobi[DSS_ * ipoint + SS_], S_, S_, S_);
        }
        {
          ...
          for (int i = 0; i < ns_; i++)
            dx_n[i] = aDual(DS_, neighbor_div_w[ind++], 2 * i);
          const aDual ux_n(DS_, neighbor_div_w[ind++], 2 * ns_);
          const aDual vx_n(DS_, neighbor_div_w[ind++], 2 * (ns_ + 1));
          const aDual Ttrx_n(DS_, neighbor_div_w[ind++], 2 * (ns_ + 2));
          const aDual Teevx_n(DS_, neighbor_div_w[ind++], 2 * (ns_ + 3));
          std::vector<aDual> dy_n(ns_, aDual(DS_));
          for (int i = 0; i < ns_; i++)
            dy_n[i] = aDual(DS_, neighbor_div_w[ind++], 2 * i + 1);
          const aDual uy_n(DS_, neighbor_div_w[ind++], 2 * ns_ + 1);
          const aDual vy_n(DS_, neighbor_div_w[ind++], 2 * (ns_ + 1) + 1);
          const aDual Ttry_n(DS_, neighbor_div_w[ind++], 2 * (ns_ + 2) + 1);
          const aDual Teevy_n(DS_, neighbor_div_w[ind], 2 * (ns_ + 3) + 1);
            ...
            for (int idim = 0; idim < D_; idim++)
              for (int jstate = 0; jstate < S_; jstate++)
                for (int jdim = 0; jdim < D_; jdim++) {
                  ...
                  for (int kstate = 0; kstate < S_; kstate++)
                    sum +=
                        flux_pri_grad_jacobi[irow * DS_ + kstate * D_ + jdim] *
                        neighbor_pri_jacobi[ipoint * SS_ + kstate * S_ +
                                            jstate];

                  flux_neighbor_grad_jacobi[DDSS_ * ipoint + irow * DS_ +
                                            icolumn] = sum;
                }
          for (int istate = 0; istate < S_; istate++)
            for (int idim = 0; idim < D_; idim++) {
...
// ------------------------------- Boundary -------------------------------- //
std::shared_ptr<BoundaryNS2DNeq2Tndcs> BoundaryNS2DNeq2Tndcs::GetBoundary(
    const std::string& type, const int bdry_tag,
    EquationNS2DNeq2Tndcs* equation) {
  if (!type.compare("SlipWall"))
    return std::make_shared<SlipWallNS2DNeq2Tndcs>(bdry_tag, equation);
  else if (!type.compare("IsothermalWall"))
    return std::make_shared<IsothermalWallNS2DNeq2Tndcs>(bdry_tag, equation);
  else if (!type.compare("LimitingIsothermalWall"))
    return std::make_shared<LimitingIsothermalWallNS2DNeq2Tndcs>(bdry_tag, equation);
  else if (!type.compare("AdiabaticWall"))
    return std::make_shared<AdiabaticWallNS2DNeq2Tndcs>(bdry_tag, equation);
  else if (!type.compare("SupersonicInflow"))
    return std::make_shared<SupersonicInflowBdryNS2DNeq2Tndcs>(bdry_tag,
                                                               equation);
  else if (!type.compare("SupersonicOutflow"))
    return std::make_shared<SupersonicOutflowBdryNS2DNeq2Tndcs>(bdry_tag,
                                                                equation);
  else if (!type.compare("Farfield"))
    return std::make_shared<FarfieldBdryNS2DNeq2Tndcs>(bdry_tag,
                                                                equation);
  else if (!type.compare("CatalyticWall"))
    return std::make_shared<CatalyticWallNS2DNeq2Tndcs>(bdry_tag, equation);
  else if (!type.compare("SuperCatalyticWall"))
    return std::make_shared<SuperCatalyticWallNS2DNeq2Tndcs>(bdry_tag,
                                                             equation);
...
// Dependency: -
SlipWallNS2DNeq2Tndcs::SlipWallNS2DNeq2Tndcs(const int bdry_tag,
                                             EquationNS2DNeq2Tndcs* equation)
    : BoundaryNS2DNeq2Tndcs(bdry_tag, equation) {
  auto& config = AVOCADO_CONFIG;
  scale_ =
      std::stod(config->GetConfigValue(BDRY_INPUT_I(bdry_tag, 0)));
...
void SlipWallNS2DNeq2Tndcs::ComputeBdrySolution(
  ...
  std::vector<double> owner_w(num_points * S_, 0.0);
  std::vector<double> bdry_w(num_points * S_, 0.0);
  int ind = 0;
  sol2pri(num_points, &owner_u[0], &owner_w[0]);
  std::vector<double> dim_d(ns_);
  for (int ipoint = 0; ipoint < num_points; ipoint++) {
    ...
    // ps
    ind = S_ * ipoint;
    for (int i = 0; i < ns_; i++) bdry_w[ind++] = d[i];
    bdry_w[ind++] = u_new;
    bdry_w[ind++] = v_new;
    bdry_w[ind++] = T_tr;
    bdry_w[ind] = T_eev;
  }
  pri2sol(num_points, &bdry_w[0], &bdry_u[0]);
  // Don't touch bdry_div_u.
}
void SlipWallNS2DNeq2Tndcs::ComputeBdryFlux(const int num_points,
                                            std::vector<double>& flux,
                                            FACE_INPUTS,
...
void SlipWallNS2DNeq2Tndcs::ComputeBdrySolutionJacobi(
  ...
  static std::vector<double> bdry_cons(S_, 0.0);
  std::vector<double> owner_w(num_points * S_, 0.0);
  std::vector<double> pri_jacobi(num_points * SS_, 0.0);
  static std::vector<double> sol_jacobi(SS_, 0.0);
  static std::vector<double> bdry_w_jacobi(SS_, 0.0);
  static std::vector<double> temp_jacobi(SS_, 0.0);
  ...
  for (int ipoint = 0; ipoint < num_points; ipoint++) {
    ...
    ind = S_ * ipoint;
    std::vector<aDual> d(ns_, aDual(S_));
    for (int i = 0; i < ns_; i++) d[i] = aDual(S_, owner_w[ind++], i);
    aDual u(S_, owner_w[ind++], ns_);
    aDual v(S_, owner_w[ind++], ns_ + 1);
    aDual T_tr(S_, owner_w[ind++], ns_ + 2);
    aDual T_eev(S_, owner_w[ind], ns_ + 3);
    ...
    for (int i = 0; i < S_; i++) bdry_cons[i] = bdry_sol[i].f;

    ConsJacobian(1, &bdry_cons[0], &sol_jacobi[0]);

    ind = 0;
    for (int i = 0; i < S_; i++)
      for (int j = 0; j < S_; j++) bdry_w_jacobi[ind++] = bdry_sol[i].df[j];

    gemmAB(1.0, &sol_jacobi[0], &bdry_w_jacobi[0], 0.0,
        &temp_jacobi[0], S_, S_, S_);

    gemmAB(1.0, &temp_jacobi[0], &pri_jacobi[SS_ * ipoint], 0.0,
      &bdry_u_jacobi[SS_ * ipoint], S_, S_, S_);
  }
}
void SlipWallNS2DNeq2Tndcs::ComputeBdryFluxJacobi(
    const int num_points, std::vector<double>& flux_jacobi,
  ...
  //      const auto Cv_tr =
  //          species[i]->GetTransRotationSpecificHeat(dim_T_tr.f) * T_ref_ /
  //          e_ref_;
  //      const auto Cv_eev =
...
// Dependency: Twall
IsothermalWallNS2DNeq2Tndcs::IsothermalWallNS2DNeq2Tndcs(
    const int bdry_tag, EquationNS2DNeq2Tndcs* equation)
    : BoundaryNS2DNeq2Tndcs(bdry_tag, equation) {
  auto& config = AVOCADO_CONFIG;
  scale_ =
      std::stod(config->GetConfigValue(BDRY_INPUT_I(bdry_tag, 0)));
...
void IsothermalWallNS2DNeq2Tndcs::ComputeBdrySolution(
    const int num_points, std::vector<double>& bdry_u,
    std::vector<double>& bdry_div_u, const std::vector<double>& owner_u,
    const std::vector<double>& owner_div_u, const std::vector<double>& normal,
    const std::vector<double>& coords, const double& time) {
  ...
  for (int ipoint = 0; ipoint < num_points; ipoint++) {
    ...
    // ps
    ind = S_ * ipoint;
    //for (int i = 0; i < ns_; i++) bdry_w[ind++] = Y[i] * d_wall;
    for (int i = 0; i < ns_; i++) bdry_w[ind++] = d[i];
    bdry_w[ind++] = -scale_*u;
    bdry_w[ind++] = -scale_*v;
    bdry_w[ind++] = (1.0 - scale_) * (T_tr - Twall_) + Twall_;
    bdry_w[ind] = (1.0 - scale_) * (T_eev - Twall_) + Twall_;
  }
  pri2sol(num_points, &bdry_w[0], &bdry_u[0]);
  // Don't touch bdry_div_u.
}
void IsothermalWallNS2DNeq2Tndcs::ComputeBdryFlux(
  ...
  //  const double k_eev =
  //      mixture_->GetElectronicVibrationConductivity(dim_Twall, dim_Twall) /
  //      k_ref_;
  ...
  //}
  equation_->ComputeNumFlux(num_points, flux, owner_cell, -1, owner_u,
                            owner_div_u, neighbor_u, owner_div_u, normal); // for debug
}
void IsothermalWallNS2DNeq2Tndcs::ComputeBdrySolutionJacobi(
    const int num_points, double* bdry_u_jacobi,
    const std::vector<double>& owner_u, const std::vector<double>& owner_div_u,
    const std::vector<double>& normal, const std::vector<double>& coords,
    const double& time) {
  ...
  for (int ipoint = 0; ipoint < num_points; ipoint++) {
    ...
    gemmAB(1.0, &sol_jacobi[0], &bdry_w_jacobi[0], 0.0, &temp_jacobi[0], S_, S_,
           S_);
...
void IsothermalWallNS2DNeq2Tndcs::ComputeBdryFluxJacobi(
    const int num_points, std::vector<double>& flux_jacobi,
    std::vector<double>& flux_grad_jacobi, FACE_INPUTS,
    const std::vector<double>& coords, const double& time) {
  // flux_jacobi(ds1s2) = F(ds1) over U(s2)
  ...
  //    const auto mixture_d_wall = mixture_p / sum;
  //    for (int i = 0; i < ns_; i++)
  //      d_new[i] = Y[i].f * mixture_d_wall.f * rho_ref_;
  //    mixture_->SetDensity(&d_new[0]);
...
// Dependency: scale, Twall
LimitingIsothermalWallNS2DNeq2Tndcs::LimitingIsothermalWallNS2DNeq2Tndcs(
  const int bdry_tag, EquationNS2DNeq2Tndcs* equation)
  : BoundaryNS2DNeq2Tndcs(bdry_tag, equation) {
  auto& config = AVOCADO_CONFIG;
  scale_ =
...
void LimitingIsothermalWallNS2DNeq2Tndcs::ComputeBdrySolution(
  const int num_points, std::vector<double>& bdry_u,
  std::vector<double>& bdry_div_u, const std::vector<double>& owner_u,
  const std::vector<double>& owner_div_u, const std::vector<double>& normal,
  const std::vector<double>& coords, const double& time) {
...
void LimitingIsothermalWallNS2DNeq2Tndcs::ComputeBdryFlux(
  const int num_points, std::vector<double>& flux, FACE_INPUTS,
  const std::vector<double>& coords, const double& time) {
  static std::vector<double> Ds(ns_, 0.0);
  static std::vector<double> Isx(ns_, 0.0);
...
void LimitingIsothermalWallNS2DNeq2Tndcs::ComputeBdrySolutionJacobi(
  const int num_points, double* bdry_u_jacobi,
  const std::vector<double>& owner_u, const std::vector<double>& owner_div_u,
  const std::vector<double>& normal, const std::vector<double>& coords,
  const double& time) {
...
void LimitingIsothermalWallNS2DNeq2Tndcs::ComputeBdryFluxJacobi(
  const int num_points, std::vector<double>& flux_jacobi,
  std::vector<double>& flux_grad_jacobi, FACE_INPUTS,
  const std::vector<double>& coords, const double& time) {
  // flux_jacobi(ds1s2) = F(ds1) over U(s2)
...
// Dependency: Twall
AdiabaticWallNS2DNeq2Tndcs::AdiabaticWallNS2DNeq2Tndcs(
    const int bdry_tag, EquationNS2DNeq2Tndcs* equation)
    : BoundaryNS2DNeq2Tndcs(bdry_tag, equation) {
  auto& config = AVOCADO_CONFIG;
...
void AdiabaticWallNS2DNeq2Tndcs::ComputeBdrySolution(
    const int num_points, std::vector<double>& bdry_u,
    std::vector<double>& bdry_div_u, const std::vector<double>& owner_u,
    const std::vector<double>& owner_div_u, const std::vector<double>& normal,
    const std::vector<double>& coords, const double& time) {
...
void AdiabaticWallNS2DNeq2Tndcs::ComputeBdryFlux(
    const int num_points, std::vector<double>& flux, FACE_INPUTS,
    const std::vector<double>& coords, const double& time) {
  //static std::vector<double> Ds(ns_, 0.0);
  //static std::vector<double> Isx(ns_, 0.0);
...
void AdiabaticWallNS2DNeq2Tndcs::ComputeBdrySolutionJacobi(
    const int num_points, double* bdry_u_jacobi,
    const std::vector<double>& owner_u, const std::vector<double>& owner_div_u,
    const std::vector<double>& normal, const std::vector<double>& coords,
    const double& time) {
...
void AdiabaticWallNS2DNeq2Tndcs::ComputeBdryFluxJacobi(
    const int num_points, std::vector<double>& flux_jacobi,
    std::vector<double>& flux_grad_jacobi, FACE_INPUTS,
    const std::vector<double>& coords, const double& time) {
  // flux_jacobi(ds1s2) = F(ds1) over U(s2)
  ...
  //    // Viscous flux // need check -> this assumes the dwall_ to be independent
  //    // / check useless computations to solution variables
  //    const auto mu = mixture_->GetViscosity(dim_T_tr.f, dim_T_eev.f) / mu_ref_;
...
SupersonicInflowBdryNS2DNeq2Tndcs::SupersonicInflowBdryNS2DNeq2Tndcs(
    const int bdry_tag, EquationNS2DNeq2Tndcs* equation)
    : BoundaryNS2DNeq2Tndcs(bdry_tag, equation) {
...
void SupersonicInflowBdryNS2DNeq2Tndcs::ComputeBdrySolution(
    const int num_points, std::vector<double>& bdry_u,
    std::vector<double>& bdry_div_u, const std::vector<double>& owner_u,
    const std::vector<double>& owner_div_u, const std::vector<double>& normal,
    const std::vector<double>& coords, const double& time) {
  ...
  for (int ipoint = 0; ipoint < num_points; ipoint++) {
    // ps
    ind = S_ * ipoint;
    for (int i = 0; i < ns_; i++) bdry_w[ind++] = d_[i];
    bdry_w[ind++] = u_;
    bdry_w[ind++] = v_;
    bdry_w[ind++] = T_tr_;
    bdry_w[ind] = T_eev_;
...
void SupersonicInflowBdryNS2DNeq2Tndcs::ComputeBdryFlux(
    const int num_points, std::vector<double>& flux, FACE_INPUTS,
    const std::vector<double>& coords, const double& time) {
  int ind = 0;
  // convection only
...
void SupersonicInflowBdryNS2DNeq2Tndcs::ComputeBdrySolutionJacobi(
    const int num_points, double* bdry_u_jacobi,
    const std::vector<double>& owner_u, const std::vector<double>& owner_div_u,
    const std::vector<double>& normal, const std::vector<double>& coords,
    const double& time) {
...
void SupersonicInflowBdryNS2DNeq2Tndcs::ComputeBdryFluxJacobi(
    const int num_points, std::vector<double>& flux_jacobi,
    std::vector<double>& flux_grad_jacobi, FACE_INPUTS,
    const std::vector<double>& coords, const double& time) {
...
// BdryInput() = -
SupersonicOutflowBdryNS2DNeq2Tndcs::SupersonicOutflowBdryNS2DNeq2Tndcs(
    const int bdry_tag, EquationNS2DNeq2Tndcs* equation)
    : BoundaryNS2DNeq2Tndcs(bdry_tag, equation) {
  MASTER_MESSAGE("SupersonicOutflow boundary (tag=" + std::to_string(bdry_tag) +
                 ")\n");
}
void SupersonicOutflowBdryNS2DNeq2Tndcs::ComputeBdrySolution(
    const int num_points, std::vector<double>& bdry_u,
    std::vector<double>& bdry_div_u, const std::vector<double>& owner_u,
    const std::vector<double>& owner_div_u, const std::vector<double>& normal,
    const std::vector<double>& coords, const double& time) {
...
void SupersonicOutflowBdryNS2DNeq2Tndcs::ComputeBdryFlux(
    const int num_points, std::vector<double>& flux, FACE_INPUTS,
    const std::vector<double>& coords, const double& time) {
  static std::vector<double> Ds(ns_, 0.0);
  static std::vector<double> Isx(ns_, 0.0);
...
void SupersonicOutflowBdryNS2DNeq2Tndcs::ComputeBdrySolutionJacobi(
    const int num_points, double* bdry_u_jacobi,
    const std::vector<double>& owner_u, const std::vector<double>& owner_div_u,
    const std::vector<double>& normal, const std::vector<double>& coords,
    const double& time) {
...
void SupersonicOutflowBdryNS2DNeq2Tndcs::ComputeBdryFluxJacobi(
    const int num_points, std::vector<double>& flux_jacobi,
    std::vector<double>& flux_grad_jacobi, FACE_INPUTS,
    const std::vector<double>& coords, const double& time) {
  static std::vector<double> Ds(ns_, 0.0);
...
// Boundary = Farfield (Blazek)
// BdryInput() = rho_1, ..., rho_ns, u, v, T_tr, T_eev
FarfieldBdryNS2DNeq2Tndcs::FarfieldBdryNS2DNeq2Tndcs(
    const int bdry_tag, EquationNS2DNeq2Tndcs* equation)
    : BoundaryNS2DNeq2Tndcs(bdry_tag, equation) {
  MASTER_MESSAGE("Farfield boundary (tag=" + std::to_string(bdry_tag) + ")\n");
  ...
  MASTER_MESSAGE("Farfield (tag=" + std::to_string(bdry_tag) +
                 ")"
                 "\n\tdensity = " +
                 std::to_string(mixture_d_ * rho_ref_) +
                 " kg/m3"
...
void FarfieldBdryNS2DNeq2Tndcs::ComputeBdrySolution(
    const int num_points, std::vector<double>& bdry_u,
    std::vector<double>& bdry_div_u, const std::vector<double>& owner_u,
    const std::vector<double>& owner_div_u, const std::vector<double>& normal,
    const std::vector<double>& coords, const double& time) {
  ...
  for (int ipoint = 0; ipoint < num_points; ipoint++) {
    ...
    } else // subsonic
    {
      if (V >= 0.0) {
        // subsonic outflow
        const double p_b = mixture_p_;

        std::vector<double> d_b(ns_, 0.0);
        for (int i = 0; i < ns_; i++)
          d_b[i] = d[i] + (p_b - mixture_p) / (a * a);

        double temp_sum = 0.0;
        for (int i = 0; i < ns_; i++)
          temp_sum += d_b[i] * species[i]->GetSpecificGasConstant();
        temp_sum *= T_ref_ / (v_ref_ * v_ref_);

        for (int i = 0; i < ns_; i++)
          bdry_w[ind++] = d[i] + (p_b - mixture_p) / (a * a);
        bdry_w[ind++] = u + nx * (mixture_p - p_b) / (mixture_d * a);
        bdry_w[ind++] = v + ny * (mixture_p - p_b) / (mixture_d * a);
        bdry_w[ind++] = p_b / temp_sum;
        bdry_w[ind] = p_b / temp_sum;
      } else {
        // subsonic inflow
        const double p_b =
            0.5 * (mixture_p_ + mixture_p -
                   mixture_d * a * (nx * (u_ - u) + ny * (v_ - v)));

        std::vector<double> d_b(ns_, 0.0);
        for (int i = 0; i < ns_; i++)
          d_b[i] = d_[i] + (p_b - mixture_p_) / (a * a);
        ...
        for (int i = 0; i < ns_; i++) bdry_w[ind++] = d_b[i];
        bdry_w[ind++] = u_ - nx * (mixture_p_ - p_b) / (mixture_d * a);
        bdry_w[ind++] = v_ - ny * (mixture_p_ - p_b) / (mixture_d * a);
        bdry_w[ind++] = p_b / temp_sum;
        bdry_w[ind] = p_b / temp_sum;
      }
    }
...
void FarfieldBdryNS2DNeq2Tndcs::ComputeBdryFlux(
    const int num_points, std::vector<double>& flux, FACE_INPUTS,
    const std::vector<double>& coords, const double& time) {
  equation_->ComputeNumFlux(num_points, flux, owner_cell, -1, owner_u,
                            owner_div_u, neighbor_u, owner_div_u, normal);
}
void FarfieldBdryNS2DNeq2Tndcs::ComputeBdrySolutionJacobi(
    const int num_points, double* bdry_u_jacobi,
    const std::vector<double>& owner_u, const std::vector<double>& owner_div_u,
    const std::vector<double>& normal, const std::vector<double>& coords,
    const double& time) {
  static std::vector<aDual> bdry_sol(S_, aDual(S_));
  static std::vector<double> bdry_pri(S_, 0.0);
  std::vector<double> owner_w(num_points * S_, 0.0);
  std::vector<double> pri_jacobi(num_points * SS_, 0.0);
  static std::vector<double> sol_jacobi(SS_, 0.0);
  static std::vector<double> bdry_w_jacobi(SS_, 0.0);
  ...
  for (int ipoint = 0; ipoint < num_points; ipoint++) {
    ...
    const double beta = mixture_->GetBeta(dim_T_tr.f);
    const double a = std::sqrt((1.0 + beta) * mixture_p.f / mixture_d.f);
    const double V = u.f * nx + v.f * ny;
    const double Maf = Vf / a;
    ...
    if (std::abs(Maf) >= 1.0) {
      if (V >= 0.0) {
        ...
        // supersonic inflow
        for (int i = 0; i < ns_; i++) bdry_sol[ind++] = aDual(S_, d_[i]);
        bdry_sol[ind++] = aDual(S_, u_);
        bdry_sol[ind++] = aDual(S_, v_);
        bdry_sol[ind++] = aDual(S_, T_tr_);
        bdry_sol[ind] = aDual(S_, T_eev_);
      }
    } else  // subsonic
    {
      if (V >= 0.0) {
        // subsonic outflow
        const aDual p_b(S_, mixture_p_);

        std::vector<aDual> d_b(ns_, aDual(S_));
        for (int i = 0; i < ns_; i++)
          d_b[i] = d[i] + (p_b - mixture_p) / (a * a);

        aDual temp_sum = aDual(S_);
        for (int i = 0; i < ns_; i++)
          temp_sum = temp_sum + d_b[i] * species[i]->GetSpecificGasConstant();
        temp_sum = temp_sum * T_ref_ / (v_ref_ * v_ref_);

        for (int i = 0; i < ns_; i++)
          bdry_sol[ind++] = d[i] + (p_b - mixture_p) / (a * a);
        bdry_sol[ind++] = u + nx * (mixture_p - p_b) / (mixture_d * a);
        bdry_sol[ind++] = v + ny * (mixture_p - p_b) / (mixture_d * a);
        bdry_sol[ind++] = p_b / temp_sum;
        bdry_sol[ind] = p_b / temp_sum;
      } else {
        // subsonic inflow
        const aDual p_b =
        ...
        for (int i = 0; i < ns_; i++) bdry_sol[ind++] = d_b[i];
        bdry_sol[ind++] = u_ - nx * (mixture_p_ - p_b) / (mixture_d * a);
        bdry_sol[ind++] = v_ - ny * (mixture_p_ - p_b) / (mixture_d * a);
        bdry_sol[ind++] = p_b / temp_sum;
        bdry_sol[ind] = p_b / temp_sum;
      }
    }

    for (int i = 0; i < S_; i++) bdry_pri[i] = bdry_sol[i].f;
    ConsJacobian(1, &bdry_pri[0], &sol_jacobi[0]);
...
void FarfieldBdryNS2DNeq2Tndcs::ComputeBdryFluxJacobi(
    const int num_points, std::vector<double>& flux_jacobi,
    std::vector<double>& flux_grad_jacobi, FACE_INPUTS,
    const std::vector<double>& coords, const double& time) {
  static const int max_num_bdry_points = equation_->GetMaxNumBdryPoints();
...
CatalyticWallNS2DNeq2Tndcs::CatalyticWallNS2DNeq2Tndcs(
    const int bdry_tag, EquationNS2DNeq2Tndcs* equation)
    : BoundaryNS2DNeq2Tndcs(bdry_tag, equation) {
  MASTER_MESSAGE("CatalyticWall (tag=" + std::to_string(bdry_tag) + ")\n");
  ERROR_MESSAGE("CatalyticWall is not supported in this version!!!");
}
void CatalyticWallNS2DNeq2Tndcs::ComputeBdrySolution(
    const int num_points, std::vector<double>& bdry_u,
    std::vector<double>& bdry_div_u, const std::vector<double>& owner_u,
    const std::vector<double>& owner_div_u, const std::vector<double>& normal,
    const std::vector<double>& coords, const double& time) {}
void CatalyticWallNS2DNeq2Tndcs::ComputeBdryFlux(
    const int num_points, std::vector<double>& flux, FACE_INPUTS,
    const std::vector<double>& coords, const double& time) {}
void CatalyticWallNS2DNeq2Tndcs::ComputeBdrySolutionJacobi(
    const int num_points, double* bdry_u_jacobi,
    const std::vector<double>& owner_u, const std::vector<double>& owner_div_u,
    const std::vector<double>& normal, const std::vector<double>& coords,
    const double& time) {}
void CatalyticWallNS2DNeq2Tndcs::ComputeBdryFluxJacobi(
    const int num_points, std::vector<double>& flux_jacobi,
    std::vector<double>& flux_grad_jacobi, FACE_INPUTS,
    const std::vector<double>& coords, const double& time) {}
// Boundary Name = SuperCatalyticWall
// BdryInput() = Twall, rho_1, ..., rho_ns
SuperCatalyticWallNS2DNeq2Tndcs::SuperCatalyticWallNS2DNeq2Tndcs(
    const int bdry_tag, EquationNS2DNeq2Tndcs* equation)
    : BoundaryNS2DNeq2Tndcs(bdry_tag, equation) {
...
void SuperCatalyticWallNS2DNeq2Tndcs::ComputeBdrySolution(
    const int num_points, std::vector<double>& bdry_u,
    std::vector<double>& bdry_div_u, const std::vector<double>& owner_u,
    const std::vector<double>& owner_div_u, const std::vector<double>& normal,
    const std::vector<double>& coords, const double& time) {}
void SuperCatalyticWallNS2DNeq2Tndcs::ComputeBdryFlux(
    const int num_points, std::vector<double>& flux, FACE_INPUTS,
    const std::vector<double>& coords, const double& time) {}
void SuperCatalyticWallNS2DNeq2Tndcs::ComputeBdrySolutionJacobi(
    const int num_points, double* bdry_u_jacobi,
    const std::vector<double>& owner_u, const std::vector<double>& owner_div_u,
    const std::vector<double>& normal, const std::vector<double>& coords,
    const double& time) {}
void SuperCatalyticWallNS2DNeq2Tndcs::ComputeBdryFluxJacobi(
    const int num_points, std::vector<double>& flux_jacobi,
    std::vector<double>& flux_grad_jacobi, FACE_INPUTS,
    const std::vector<double>& coords, const double& time) {}
// -------------------------------- Problem -------------------------------- //
std::shared_ptr<ProblemNS2DNeq2Tndcs> ProblemNS2DNeq2Tndcs::GetProblem(
    const std::string& name) {
  if (!name.compare("FreeStream"))
    return std::make_shared<FreeStreamNS2DNeq2Tndcs>();
  else if (!name.compare("DoubleSine"))
    return std::make_shared<DoubleSineNS2DNeq2Tndcs>();
  else if (!name.compare("Test"))
    return std::make_shared<TestNS2DNeq2Tndcs>();
  else
...
FreeStreamNS2DNeq2Tndcs::FreeStreamNS2DNeq2Tndcs() {
  ...
  const double V = std::sqrt(u_ * u_ + v_ * v_);
  ...
  const double mu = mixture_->GetViscosity(T_tr_, T_eev_);
  ...
  str << "\t\tReynolds number = " << mixture_d * V * L_ref_ / mu << "\n";
...
void FreeStreamNS2DNeq2Tndcs::Problem(const int num_points,
                                      std::vector<double>& solutions,
                                      const std::vector<double>& coord,
                                      const double time) const {
  std::vector<double> pri_solutions(num_points * S_, 0.0);
  
  int ind = 0;  
  for (int ipoint = 0; ipoint < num_points; ipoint++) {
    for (int i = 0; i < ns_; i++) pri_solutions[ind++] = d_[i];
    pri_solutions[ind++] = u_;
    pri_solutions[ind++] = v_;
    pri_solutions[ind++] = T_tr_;
    pri_solutions[ind++] = T_eev_;
  }
  pri2sol(num_points, &pri_solutions[0], &solutions[0]);
}
// Problem = DoubleSine
// ProblemInput = -
DoubleSineNS2DNeq2Tndcs::DoubleSineNS2DNeq2Tndcs() : wave_number_({1.0, 1.0}) {
...
void DoubleSineNS2DNeq2Tndcs::Problem(const int num_points,
                                std::vector<double>& solutions,
                                const std::vector<double>& coord,
                                const double time) const {
  int ind = 0;
  ...
  for (int ipoint = 0; ipoint < num_points; ipoint++) {
    ...
    ind = S_ * ipoint;
    pri_solutions[ind] = values[0];
    ind += ns_;
    for (int idim = 0; idim < D_; idim++)
      pri_solutions[ind++] = values[idim + 1];
    pri_solutions[ind++] = v_ref_ * v_ref_ / values[0] / R1 / T_ref_;
    pri_solutions[ind] = values[3];
  }
  pri2sol(num_points, &pri_solutions[0], &solutions[0]);
}
// Problem = Test
// ProblemInput = -
TestNS2DNeq2Tndcs::TestNS2DNeq2Tndcs() : wave_number_({1.0, 1.0}) {
  MASTER_MESSAGE("Test problem: DoubleSine\n");
...
void TestNS2DNeq2Tndcs::Problem(const int num_points,
                                std::vector<double>& solutions,
                                const std::vector<double>& coord,
                                const double time) const {
  int ind = 0;
  ...
  for (int ipoint = 0; ipoint < num_points; ipoint++) {
    ...
    const double& y = coord[ipoint * D_ + 1];
     ...
     if (y > 0.5) {
      ...
      pri_solutions[ind++] = u_;
      pri_solutions[ind++] = v_;
      pri_solutions[ind++] = T_tr_ * (1.5 - y);
      pri_solutions[ind++] = T_eev_ * (1.5 - y);
    } else {
      for (int i = 0; i < ns_; i++) pri_solutions[ind++] = d_[i];
      pri_solutions[ind++] = u_;
      pri_solutions[ind++] = v_;
      pri_solutions[ind++] = T_tr_ * (0.5 + y);
      pri_solutions[ind++] = T_eev_ * (0.5 + y);
    }    
  }
  pri2sol(num_points, &pri_solutions[0], &solutions[0]);
}
